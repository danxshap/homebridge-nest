...
class NestThermostatAccessory extends NestDeviceAccessory {
    ...
    setActiveStatus(state, callback) {
        try {
            // Reject attempts to turn OFF an active thermostat sensor (radio button behavior)
            if (!state && this.getActiveStatus()) {
                this.log.info("Rejecting attempt to turn OFF active thermostat sensor '" + this.name + "' - use radio button behavior");
                callback(null); // Complete the HomeKit request first
                
                // Revert the switch back to ON state with a slight delay
                setTimeout(() => {
                    let switchSubtype = 'switch.' + this.device.serial_number;
                    let sensorSwitchService = this.accessory.getService(Service.Switch, switchSubtype);
                    if (sensorSwitchService) {
                        sensorSwitchService.getCharacteristic(Characteristic.On).updateValue(true);
                        this.log.info("Reverted thermostat sensor '" + this.name + "' switch back to ON");
                    }
                }, 100);
                return;
            }
            
            this.log.info('Using thermostat ' + this.name + ' for RCS: ' + state);
            this.setPropertyAsync('rcs_settings', 'active_rcs_sensors', {thermostat: this.device.device_id, sensor: null, state: state}, 'active sensor switch').asCallback((err, result) => {
                if (err) {
                    this.log.error("Error setting active sensor state:", err.message || err);
                    callback(err);
                } else {
            ...
            this.log.error("Exception in setActiveStatus:", error);
            callback(error);
        }
    }

    updateData(device, structure) {
        let previousDevice = this.device;
        
        // Call parent updateData first
        super.updateData(device, structure);
        
        // Handle sensor switch updates specific to thermostats
        if (device && this.accessory) {
            let previousActive = previousDevice ? previousDevice.active_sensor : undefined;
            let currentActive = device.active_sensor;
            
            // Only update if the active status changed
            if (previousActive !== currentActive) {
                this.log.info('Thermostat ' + this.device.name + ' active sensor state changed from Nest: ' + previousActive + ' -> ' + currentActive);
                
                // Always update this thermostat's sensor switch state
                let switchSubtype = 'switch.' + this.device.serial_number;
                let sensorSwitchService = this.accessory.getService(Service.Switch, switchSubtype);
                if (sensorSwitchService) {
                    this.log.info('Updating thermostat sensor switch to: ' + currentActive);
                    sensorSwitchService.getCharacteristic(Characteristic.On).updateValue(currentActive);
                } else {
                    this.log.error('Could not find thermostat sensor switch service with subtype: ' + switchSubtype);
                    // Debug: List all switch services
                    let allSwitches = this.accessory.services.filter(s => s.UUID === Service.Switch.UUID);
                    this.log.error('Available switch services: ' + allSwitches.map(s => s.subtype || 'no-subtype').join(', '));
                }
            }
        }
    }
