...
class NestTempSensorAccessory extends NestDeviceAccessory {
    ...
    setActiveStatus(state, callback) {
        try {
            // Reject attempts to turn OFF an active sensor (radio button behavior)
            if (!state && this.getActiveStatus()) {
                this.log.info("Rejecting attempt to turn OFF active temperature sensor '" + this.device.name + "' - use radio button behavior");
                callback(null); // Complete the HomeKit request first
                
                // Revert the switch back to ON state with a slight delay
                setTimeout(() => {
                    let switchSubtype = 'switch.' + this.device.device_id;
                    ...
                    if (sensorSwitchService) {
                        ...
                        this.log.info("Reverted temperature sensor '" + this.device.name + "' switch back to ON");
                    }
                }, 100);
                return;
            }
            
            this.log.info("Setting Nest Temperature Sensor '" + this.device.name + "' active state to " + state);
            this.setPropertyAsync('rcs_settings', 'active_rcs_sensors', {thermostat: this.device.thermostat_device_id, sensor: this.device.device_id, state: state}, 'active sensor switch').asCallback((err, result) => {
                if (err) {
    ...
    updateData(device, structure) {
        ...
        // Handle sensor switch updates specific to temperature sensors
        if (device && this.accessory) {
            let previousActive = previousDevice ? previousDevice.active : undefined;
            let currentActive = device.active;
            
            // Only update if the active status changed
            if (previousActive !== currentActive) {
                this.log.info('Temperature sensor ' + this.device.name + ' active state changed from Nest: ' + previousActive + ' -> ' + currentActive);
                
                // Always update this sensor's switch state
                let switchSubtype = 'switch.' + this.device.device_id;
                let sensorSwitchService = this.accessory.getService(Service.Switch, switchSubtype);
                if (sensorSwitchService) {
                    sensorSwitchService.getCharacteristic(Characteristic.On).updateValue(currentActive);
                } else {
                    this.log.error('Could not find switch service for ' + this.device.name + ' with subtype: ' + switchSubtype);
                }
            }
        }
    }
