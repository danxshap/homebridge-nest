...
class NestTempSensorAccessory extends NestDeviceAccessory {
    ...
    setActiveStatus(state, callback) {
        ...
        this.setPropertyAsync('rcs_settings', 'active_rcs_sensors', {thermostat: this.device.thermostat_device_id, sensor: this.device.device_id, state: state}, 'active sensor switch').asCallback(callback);
    }
    ...
    updateData(device, structure) {
        ...
        if (device && this.accessory) {
            ...
            if (previousActive !== currentActive) {
                let switchService = this.accessory.getService(this.Service.Switch);
                if (switchService) {
                    this.log.debug('Updating temperature sensor switch state from Nest: ' + currentActive);
                    switchService.getCharacteristic(this.Characteristic.On).updateValue(currentActive);
                }
            }
        }
    }
