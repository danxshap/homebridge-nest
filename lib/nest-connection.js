...
class Connection {
    ...
    findOtherActiveSensor(thermostatId, excludeSensorId) {
        ...
        for (let deviceId in this.accessories) {
            ...
            if (accessory.device && 
                ...
                let switchService = accessory.accessory.getService(accessory.Service.Switch);
                if (switchService) {
                    let isOn = switchService.getCharacteristic(accessory.Characteristic.On).value;
                    if (isOn) {
    ...
    updateSwitchState(accessory, state) {
        try {
            let switchService = accessory.accessory.getService(accessory.Service.Switch);
            if (switchService) {
                switchService.getCharacteristic(accessory.Characteristic.On).updateValue(state);
            }
        } catch (error) {
