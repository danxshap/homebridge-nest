...
const { Service, Characteristic } = require('./nest-device-accessory')();
...
class Connection {
    ...
    findOtherActiveSensor(thermostatId, excludeSensorId) {
        ...
        for (let deviceId in this.accessories) {
            ...
            if (accessory.device && 
                ...
                let switchService = accessory.accessory.getService(Service.Switch);
                if (switchService) {
                    let isOn = switchService.getCharacteristic(Characteristic.On).value;
                    if (isOn) {
    ...
    updateSwitchState(accessory, state) {
        try {
            ...
            if (switchSubtype) {
                // Use the existing getService method which supports subtype
                sensorSwitchService = accessory.accessory.getService(Service.Switch, switchSubtype);
                if (sensorSwitchService) {
                    sensorSwitchService.getCharacteristic(Characteristic.On).updateValue(state);
                }
            }
        } catch (error) {
