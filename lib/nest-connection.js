...
class Connection {
                ...
                if (protoBuffer.length >= pendingLength) {
                    ...
                    if (notify || observeMessage.hasDeviceInfo) {
                        if (!isEmptyObject(this.protobufBody)) {
                            ...
                            // DEBUG: Log raw API response for RCS settings
                            if (body.rcs_settings) {
                                console.log('DEBUG RAW API: rcs_settings =', JSON.stringify(body.rcs_settings, null, 2));
                            }
    ...
    apiResponseToObjectTree(body) {
        // DEBUG: Log what comes into this function
        if (body.rcs_settings) {
            console.log('DEBUG INPUT TO apiResponseToObjectTree: rcs_settings =', JSON.stringify(body.rcs_settings, null, 2));
        }
