                ...
                if (object) {
                    ...
                    translateProperty(object, 'remote_comfort_sensing_settings', null, (rcsSetting, id) => {
                        ...
                        if (checkDeviceExists(body, id)) {
                            ...
                            // DEBUG: Log what we're processing from protobuf
                            console.log('DEBUG PROTOBUF: Processing RCS for thermostat', id, 'activeSensor:', activeSensor, 'from value:', JSON.stringify(value.activeRcsSelection, null, 2));
                            ...
                            if (activeSensor) {
                                ...
                                console.log('DEBUG PROTOBUF: Added activeSensor', activeSensor, 'to array, now:', body.rcs_settings[id]["active_rcs_sensors"]);
                            }
                        }
                    });
            ...
            swarm.map(unit => unit.split('.')).forEach(unit => {
                try {
                    ...
                    if (deviceType == 'device') {
                        ...
                        // BUGFIX: Enforce mutual exclusivity by using only the first active sensor
                        if (rcs_settings[deviceId] && rcs_settings[deviceId].active_rcs_sensors && rcs_settings[deviceId].active_rcs_sensors.length > 0) {
                            // Log warning if multiple sensors detected
                            if (rcs_settings[deviceId].active_rcs_sensors.length > 1) {
                                this.log('WARNING: Multiple active sensors detected for thermostat ' + deviceId + ', using only first: ' + JSON.stringify(rcs_settings[deviceId].active_rcs_sensors));
                            }
                            
                            // Use only the first active sensor to enforce mutual exclusivity
                            let firstActiveSensor = rcs_settings[deviceId].active_rcs_sensors[0];
                            let [type, sensorId, ..._] = firstActiveSensor.split('.');
                            if (type === 'kryptonite' && 
                                data.devices['temp_sensors'] && 
                                data.devices['temp_sensors'][sensorId]) {
