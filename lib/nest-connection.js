                    ...
                    translateProperty(object, 'remote_comfort_sensing_settings', null, (rcsSetting, id) => {
                        ...
                        if (checkDeviceExists(body, id)) {
                            ...
                            if (value.activeRcsSelection && 
                                ...
                                let resourceId = value.activeRcsSelection.activeRcsSensor.resourceId;
                                if (value.activeRcsSelection.rcsSourceType === "RCS_SOURCE_TYPE_SINGLE_SENSOR") {
                                    // External temperature sensor
                                    activeSensor = 'kryptonite.' + resourceId.split('_')[1];
                                } else if (value.activeRcsSelection.rcsSourceType === "RCS_SOURCE_TYPE_BACKPLATE") {
                                    // Thermostat built-in sensor
                                    activeSensor = 'device.' + resourceId.split('_')[1];
                                }
                            }
            ...
            swarm.map(unit => unit.split('.')).forEach(unit => {
                try {
                    ...
                    if (deviceType == 'device') {
                        ...
                        let baseName = (thisDevice.name || thisDevice.where_name || 'Nest').replace(/ Thermostat.*$/, '');
                        thisDevice.name = baseName + ' Thermostat';
                        ...
                        // Reset all temperature sensors for this thermostat to inactive first
                        if (rcs_settings[deviceId] && rcs_settings[deviceId].associated_rcs_sensors) {
                            rcs_settings[deviceId].associated_rcs_sensors.forEach(sensorName => {
                                let sensorId = sensorName.split('.')[1];
                                if (data.devices['temp_sensors'] && data.devices['temp_sensors'][sensorId]) {
                                    data.devices['temp_sensors'][sensorId].active = false;
                                }
                            });
                        }
                        
                        // Now set only the currently active sensor(s) to active
    ...
    update(device, property, value, hvac_mode, using_protobuf) {
        ...
        if (deviceType == 'structure') {
            ...
            if (property == 'active_rcs_sensors') {
                ...
                if (!!state) {
                    // Activating a sensor - only change which sensor is active, preserve schedule setting
                    if (sensorId !== null) {
                    ...
                    // Deactivating a sensor - fall back to thermostat built-in sensor
    ...
    mergeNestWithProtobufData(nestApiData, data) {
        ...
        for (const el in data) {
            if (typeof(data[el]) == 'object' && data[el] && !Array.isArray(data[el])) {
                ...
                // Special handling for active_rcs_sensors - replace instead of accumulate
                if (el === 'active_rcs_sensors') {
                    nestApiData[el] = [...data[el]]; // Replace the array
                } else {
                    // For other arrays, use the original accumulating behavior
                    data[el].forEach(val => {
                        if (!nestApiData[el]) {
                            nestApiData[el] = {};
                        }
        ...
        function addToOutput(output, currentState, device, protoKey, protoType, key, value) {
            ...
            if (!found) {
                ...
                if (protoType == 'type.nestlabs.com/nest.trait.hvac.TargetTemperatureSettingsTrait') {
                    ...
                    // Preserve the existing rcs_control_mode from current state
                    let existingControlMode = 'RCS_CONTROL_MODE_SCHEDULE';
                    if (currentState.rcs_settings && 
                        currentState.rcs_settings[device] && 
                        currentState.rcs_settings[device].rcs_control_mode) {
                        existingControlMode = currentState.rcs_settings[device].rcs_control_mode;
                    }
                    
                    newEl.property.value = {
                        rcsControlMode: existingControlMode,  // Preserve user's schedule preference
                        activeRcsSelection: {},  // To be set below
                        rcsControlSchedule: {},  // The web app sets this to empty
                    };
                    if (value && value.length > 0) {
                        // A temperature sensor is selected. Keep the existing control mode
                        // instead of overriding to SCHEDULE_OVERRIDE
                        ...
                        if (value[0].startsWith('kryptonite.')) {
                            ...
                            // Based on network capture, try simpler approach without rcsSourceType
                            newEl.property.value.activeRcsSensor = {
                                resourceId: 'DEVICE_' + deviceOrSensorId,
                            }
                            // Remove the activeRcsSelection wrapper for thermostat
                            delete newEl.property.value.activeRcsSelection;
                        } else {
