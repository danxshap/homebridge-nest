                ...
                try {
                    ...
                    if (deviceType == 'device') {
                        ...
                        if (rcs_settings[deviceId] && rcs_settings[deviceId].associated_rcs_sensors) {
                            rcs_settings[deviceId].associated_rcs_sensors.forEach(sensorName => {
                                ...
                                if (data.devices['temp_sensors'] && data.devices['temp_sensors'][sensorId]) {
                                    console.log('DEBUG: Resetting sensor ' + sensorId + ' to inactive for thermostat ' + deviceId);
                        ...
                        if (rcs_settings[deviceId] && rcs_settings[deviceId].active_rcs_sensors) {
                            console.log('DEBUG: Active sensors for thermostat ' + deviceId + ': ' + JSON.stringify(rcs_settings[deviceId].active_rcs_sensors));
                            rcs_settings[deviceId].active_rcs_sensors.forEach(sensorName => {
                                let [type, sensorId, ..._] = sensorName.split('.');
                                console.log('DEBUG: Processing active sensor: ' + sensorName + ', type: ' + type + ', sensorId: ' + sensorId);
                                if (type === 'kryptonite' && 
                                    data.devices['temp_sensors'] && 
                                    data.devices['temp_sensors'][sensorId]) {
                                    console.log('DEBUG: Setting sensor ' + sensorId + ' to active');
