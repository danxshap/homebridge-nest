            ...
            swarm.map(unit => unit.split('.')).forEach(unit => {
                try {
                    ...
                    if (deviceType == 'device') {
                        ...
                        // DEBUG: Track thermostat name processing
                        let originalName = thisDevice.name;
                        this.log('DEBUG MASTER: Processing thermostat deviceId=' + deviceId);
                        this.log('DEBUG MASTER: Original name="' + originalName + '", where_name="' + thisDevice.where_name + '"');
                        
                        let baseName = (thisDevice.name || thisDevice.where_name || 'Nest').replace(/ Thermostat.*$/, '');
                        thisDevice.name = baseName + ' Thermostat';
                        
                        this.log('DEBUG MASTER: Final name="' + thisDevice.name + '"');
                        if (originalName && originalName.includes('Thermostat')) {
                            this.log('DEBUG MASTER: ⚠️  WARNING: Original name already contained "Thermostat"!');
                        }
