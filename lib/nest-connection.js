...
class Connection {
            ...
            if (object && object.message && object.message.length > 0) {
                ...
                if (object) {
                    ...
                    translateProperty(object, 'remote_comfort_sensing_settings', null, (rcsSetting, id) => {
                        ...
                        if (checkDeviceExists(body, id)) {
                            let rcsSensors = [];
                            let value = rcsSetting.data.property.value;
                            try {
                                if (value.associatedRcsSensors && Array.isArray(value.associatedRcsSensors)) {
                                    rcsSensors = value.associatedRcsSensors
                                        .filter(el => el && el.deviceId && el.deviceId.resourceId)
                                        .map(el => 'kryptonite.' + el.deviceId.resourceId.split('_')[1]);
                                }
                            } catch(error) {
                                this.log('Warning: Failed to parse RCS sensors:', error.message);
                                rcsSensors = [];
                            }
                            ...
                            let activeSensor;
                            if (value.activeRcsSelection && 
                                value.activeRcsSelection.rcsSourceType === "RCS_SOURCE_TYPE_SINGLE_SENSOR" &&
                                value.activeRcsSelection.activeRcsSensor &&
                                value.activeRcsSelection.activeRcsSensor.resourceId) {
                                activeSensor = value.activeRcsSelection.activeRcsSensor.resourceId;
                                if (activeSensor) {
                                    activeSensor = 'kryptonite.' + activeSensor.split('_')[1];
                                }
                            }
                            // console.log('rcsSensors', rcsSensors);
                            body.rcs_settings[id] = { 
                                associated_rcs_sensors: rcsSensors, 
                                active_rcs_sensors: [],
                                rcs_control_mode: value.rcsControlMode || 'RCS_CONTROL_MODE_SCHEDULE'
                            };
                            if (activeSensor) {
                                body.rcs_settings[id]["active_rcs_sensors"].push(activeSensor);
                            }
                        }
                    });
                    ...
                    if (deviceType == 'device') {
                        ...
                        if (rcs_settings[deviceId] && rcs_settings[deviceId].active_rcs_sensors) {
                            rcs_settings[deviceId].active_rcs_sensors.forEach(sensorName => {
                                let [type, sensorId, ..._] = sensorName.split('.');
                                if (type === 'kryptonite' && 
                                    data.devices['temp_sensors'] && 
                                    data.devices['temp_sensors'][sensorId]) {
    ...
    update(device, property, value, hvac_mode, using_protobuf) {
        ...
        if (deviceType == 'structure') {
            ...
            if (property == 'active_rcs_sensors') {
                // Two variables are of interest here:
                // 1. sensorId: if null, it's coming from the thermostat (i.e. BACKPLATE)
                // 2. state: if true, it's being set to SCHEDULE_OVERRIDE, otherwise
                // it's being reverted to SCHEDULE
                deviceId = value.thermostat;
                let sensorId = value.sensor;
                let state = value.state;
                
                if (!!state) {
                    // Activating a sensor - set it as the only active sensor
                    body['rcs_control_setting'] = "SCHEDULE_OVERRIDE";
                    if (sensorId !== null) {
                        body['active_rcs_sensors'] = ["kryptonite." + sensorId];
                    } else {
                        body['active_rcs_sensors'] = ["device." + deviceId];
                    }
                    
                    // Update HomeKit: turn off all other sensor switches for this thermostat
                    try {
                        this.updateOtherSensorSwitches(deviceId, sensorId, false);
                    } catch (error) {
                        this.error('Error updating other sensor switches:', error);
                    }
                } else {
                    // Deactivating a sensor - need to determine what should be active instead
                    let currentControlMode = this.currentState && 
                                           this.currentState.rcs_settings && 
                                           this.currentState.rcs_settings[deviceId] && 
                                           this.currentState.rcs_settings[deviceId].rcs_control_mode;
                    
                    // Convert from protobuf format to legacy format for control mode
                    let legacyControlMode = "SCHEDULE_OVERRIDE"; // Default to override since we're manually controlling
                    if (currentControlMode === 'RCS_CONTROL_MODE_SCHEDULE') {
                        legacyControlMode = "SCHEDULE";
                    } else if (currentControlMode === 'RCS_CONTROL_MODE_HOLD') {
                        legacyControlMode = "HOLD";
                    }
                    
                    // Check if any other sensor switches are currently on
                    let otherActiveSensor = this.findOtherActiveSensor(deviceId, sensorId);
                    
                    if (otherActiveSensor) {
                        // Another sensor is active, use it
                        body = {
                            active_rcs_sensors: [otherActiveSensor],
                            rcs_control_setting: legacyControlMode
                        };
                    } else {
                        // No other sensors active, fall back to thermostat built-in sensor
                        body = {
                            active_rcs_sensors: ["device." + deviceId],
                            rcs_control_setting: legacyControlMode
                        };
                        
                        // Update HomeKit: turn on the thermostat sensor switch
                        try {
                            this.updateThermostatSensorSwitch(deviceId, true);
                        } catch (error) {
                            this.error('Error updating thermostat sensor switch:', error);
                        }
                    }
                }
            } else {
                this.error(`Unknown property for rcs_settings: ${property}`);
            }
        }
    ...
    legacyToProtobufUpdate(body) {
        function addToOutput(output, currentState, device, protoKey, protoType, key, value) {
            ...
            if (!found) {
                ...
                } else if (protoType == "type.nestlabs.com/nest.trait.hvac.RemoteComfortSensingSettingsTrait") {
                    newEl.property.value = {
                        rcsControlMode: 'RCS_CONTROL_MODE_SCHEDULE',
                        activeRcsSelection: {},  // To be set below
                        rcsControlSchedule: {},  // The web app sets this to empty
                    };
                    if (value && value.length > 0) {
                        // A temperature sensor is selected. Override RCS schedule
                        // and set sensor to the selected sensor.
                        newEl.property.value.rcsControlMode = 'RCS_CONTROL_MODE_SCHEDULE_OVERRIDE';
                        let deviceOrSensorId = value[0].split('.')[1];
                        if (value[0].startsWith('kryptonite.')) {
                            // Selected sensor is a Nest Temperature Sensor
                            newEl.property.value.activeRcsSelection = {
                                rcsSourceType: 'RCS_SOURCE_TYPE_SINGLE_SENSOR',
                                activeRcsSensor: {
                                    resourceId: 'DEVICE_' + deviceOrSensorId,
                                }
                            }
                        } else if (value[0].startsWith('device.')) {
                            // Selected sensor is the thermostat itself
                            newEl.property.value.activeRcsSelection = {
                            ...
                            this.log.error('Unknown RCS sensor type', value[0]);
                        }
                    } else {
                        // User toggled off a sensor / backplate, revert back to RCS
                        // schedule and set no active sensor
                        newEl.property.value.activeRcsSelection = {
                            rcsSourceType: 'RCS_SOURCE_TYPE_UNSPECIFIED',
                            activeRcsSensor: {}
                        }
                    }
        ...
        data.forEach(req => {
            ...
            for (const key in prop) {
                ...
                } else if (['active_rcs_sensors'].includes(key)) {
                    protoKey = 'remote_comfort_sensing_settings';
                    protoType = 'type.nestlabs.com/nest.trait.hvac.RemoteComfortSensingSettingsTrait';
                } else {
                    known = false;
                }
                if (known) {
    ...
    // Helper method to find other active sensors for a given thermostat
    findOtherActiveSensor(thermostatId, excludeSensorId) {
        if (!this.accessories) return null;
        
        // Check all accessories to find temperature sensors for this thermostat
        for (let deviceId in this.accessories) {
            let accessory = this.accessories[deviceId];
            
            // Check if this is a temperature sensor for the target thermostat
            if (accessory.device && 
                accessory.device.thermostat_device_id === thermostatId && 
                accessory.device.device_id !== excludeSensorId &&
                accessory.constructor.name === 'NestTempSensorAccessory') {
                
                // Check if this sensor's switch is currently on
                let switchSubtype = 'switch.' + accessory.device.device_id;
                let services = accessory.accessory.services || [];
                let switchService = null;
                
                for (let service of services) {
                    if (service.subtype === switchSubtype) {
                ...
                if (switchService) {
                    let onCharacteristic = switchService.characteristics.find(char => char.displayName === 'On');
                    if (onCharacteristic && onCharacteristic.value) {
                        return "kryptonite." + accessory.device.device_id;
                    }
                }
            }
        }
    ...
    // Helper method to update other sensor switches for a thermostat (turn them off when one is activated)
    updateOtherSensorSwitches(thermostatId, activatedSensorId, state) {
        if (!this.accessories) return;
        
        // Update all sensor switches for this thermostat (except the one being activated)
        for (let deviceId in this.accessories) {
            let accessory = this.accessories[deviceId];
            
            // Check temperature sensors for this thermostat
            if (accessory.device && 
                accessory.device.thermostat_device_id === thermostatId && 
                accessory.device.device_id !== activatedSensorId &&
                accessory.constructor.name === 'NestTempSensorAccessory') {
                
                this.updateSwitchState(accessory, state);
            }
            
            // Also update the thermostat's sensor switch if a sensor is being activated
            if (accessory.device && 
                accessory.device.device_id === thermostatId &&
                accessory.constructor.name === 'NestThermostatAccessory' &&
                activatedSensorId !== null) {
    ...
    // Helper method to update the thermostat's built-in sensor switch
    updateThermostatSensorSwitch(thermostatId, state) {
        if (!this.accessories) return;
        
        let thermostatAccessory = this.accessories[thermostatId];
        if (thermostatAccessory && thermostatAccessory.constructor.name === 'NestThermostatAccessory') {
            this.updateSwitchState(thermostatAccessory, state);
        }
    }

    // Helper method to update a switch characteristic state
    updateSwitchState(accessory, state) {
        try {
            // Find the sensor switch service by subtype pattern
            let sensorSwitchService = null;
            let switchSubtype = null;
            
            // Determine the subtype pattern based on accessory type
            if (accessory.constructor.name === 'NestTempSensorAccessory') {
                switchSubtype = 'switch.' + accessory.device.device_id;
            } else if (accessory.constructor.name === 'NestThermostatAccessory') {
                switchSubtype = 'switch.' + accessory.device.serial_number;
            }
            
            if (switchSubtype) {
                // Find the service by iterating through services and matching subtype
                let services = accessory.accessory.services || [];
                for (let service of services) {
                    if (service.subtype === switchSubtype && service.displayName && service.displayName.includes('Sensor')) {
                        sensorSwitchService = service;
                        break;
                    }
                }
                
                if (sensorSwitchService) {
                    // Find the 'On' characteristic
                    let onCharacteristic = sensorSwitchService.characteristics.find(char => char.displayName === 'On');
                    if (onCharacteristic) {
                        onCharacteristic.updateValue(state);
                    }
                }
            }
        } catch (error) {
            this.error('Error updating switch state:', error);
        }
    }

    // Method to handle mutual exclusivity when active sensor changes from Nest app
    handleNestActiveSensorChange(thermostatId, activeSensorId, sensorType) {
        if (!this.accessories) return;
        
        if (sensorType === 'sensor') {
            // A temperature sensor became active - turn off all others including thermostat
            this.updateOtherSensorSwitches(thermostatId, activeSensorId, false);
        } else if (sensorType === 'thermostat') {
            // Thermostat became active - turn off all temperature sensors
            this.updateOtherSensorSwitches(thermostatId, null, false);
        }
    }
}
